"""Audio encoder module for precomputed audio embeddings.

Provides temporal modeling (GRU, Transformer, Attention pooling, etc.).
Logic originally lived in the legacy MER unimodal trainer; audio-only runs now use
`multimodal_classifier.py --fusion-type audio_only`.
"""
...
class AudioEncoder(nn.Module):
    """Temporal encoder for audio features.
    ...
    def __init__(
        ...
        Args:
            ...
            model_type: one of 'gru', 'transformer', 'transformer_first', 'attn_pool', 'mean_pool'
            gru_hidden: hidden size for GRU
            gru_layers: number of GRU layers
            bidirectional: whether GRU is bidirectional
            gru_dropout: dropout for GRU
            tr_nhead: number of attention heads for transformer
            tr_layers: number of transformer encoder layers
            tr_dropout: dropout for transformer
            attn_dropout: dropout for attention pooling
            linear_dim: optional intermediate projection dimension (0 = no projection)
            activation: activation function ('relu', etc.)
    ...
    def forward(self, x: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: (B, T, input_dim) audio feature sequences
            lengths: (B,) valid lengths for each sequence
        
        Returns:
            (B, out_dim) encoded audio features
        """
        if self.model_type == "gru":
